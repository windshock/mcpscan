...
- New unknown-lab corpus: `lab/unknown/fetch.sh` downloads Flowise 3.1.0/3.0.13 (npm) and Upsonic 0.72.0/0.71.6 (PyPI) — real production releases without pre-labelled expected_findings, mounted at `/unknown` in the runner. Source-only stage (`unknown-mcp-guard`, `unknown-mcpscan`) feeds `results/report/unknown-lab.md` with per-package findings, version-pair deltas, and scanner overlap. Two main observations from the blind run: (a) zero version-pair delta — both scanners produce identical output for older vs newer release, suggesting the patterns are too coarse to surface micro-version security improvements; (b) zero scanner overlap — mcp-guard and MCPScan have completely disjoint taxonomies. Live-launch and config-mode are not applicable because neither package exposes a native MCP server CLI (Flowise is an AI workflow platform, Upsonic is an MCP *client* SDK).
